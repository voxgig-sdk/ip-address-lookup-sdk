...
func TestGetIpAddressEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IP_ADDRESS_LOOKUP_TEST_GET_IP_ADDRESS_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		getIpAddressRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_ip_address")))
...
func get_ip_addressBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IP_ADDRESS_LOOKUP_TEST_GET_IP_ADDRESS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IP_ADDRESS_LOOKUP_TEST_GET_IP_ADDRESS_ENTID"])
