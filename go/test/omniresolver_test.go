...
import (
	...
	omni "github.com/voxgig-sdk/ip-address-lookup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpAddressLookupError); is {
				out := map[string]any{
					"name":    "IpAddressLookupError",
...
func omniCtx(arg any, client *sdk.IpAddressLookupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpAddressLookupSDK); is {
			client = live
