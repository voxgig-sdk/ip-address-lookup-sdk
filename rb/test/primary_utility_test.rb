...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = IpAddressLookupBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = IpAddressLookupSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = IpAddressLookupSDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = IpAddressLookupSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = IpAddressLookupSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_fetch_def_basic
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = IpAddressLookupSpec.new({
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = IpAddressLookupSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = IpAddressLookupSDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = IpAddressLookupResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = IpAddressLookupOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = IpAddressLookupSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = IpAddressLookupContext.new(ctxmap, nil)
    ...
    if ctxmap["spec"].is_a?(Hash)
      ctx.spec = IpAddressLookupSpec.new(ctxmap["spec"])
    end

    # Handle result from JSON map
    if ctxmap["result"].is_a?(Hash)
      res_map = ctxmap["result"]
      ctx.result = IpAddressLookupResult.new(res_map)
      if res_map["err"].is_a?(Hash) && res_map["err"]["message"].is_a?(String)
        ctx.result.err = IpAddressLookupError.new("", res_map["err"]["message"])
      end
    end
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = IpAddressLookupResponse.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    IpAddressLookupError.new(code, msg)
  end
end
