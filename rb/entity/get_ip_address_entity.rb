...
class GetIpAddressEntity
  ...
  def make
    opts = @_entopts.dup
    GetIpAddressEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpAddressLookupHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetIpAddress, Hash] the current GetIpAddress data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpAddressLookupHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetIpAddress fields)
  def match_get
  ...
  #
  # @param reqmatch [GetIpAddressLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetIpAddress.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetIpAddress, Hash] the loaded GetIpAddress; raises IpAddressLookupError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpAddressLookupHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
