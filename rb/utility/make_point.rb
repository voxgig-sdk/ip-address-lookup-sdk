...
module IpAddressLookupUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(IpAddressLookupError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      op.points.each do |p|
        point = p
        select_def = IpAddressLookupHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = IpAddressLookupHelpers.to_map(VoxgigStruct.getprop(point, "select"))
