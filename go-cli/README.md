...
boru-driven command-line client **and** interactive REPL for the IpAddressLookup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-lookup-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-address-lookup-cli --help

# 3. Provide credentials once, via the environment
export IP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-address-lookup-cli load 1 get_ip_address            # {id:1} shorthand
./ip-address-lookup-cli load '{id:1}' get_ip_address       # explicit match map

# 5. Override the API base URL for a single call
IP_ADDRESS_LOOKUP_BASE=https://api.example.com ./ip-address-lookup-cli load 1 get_ip_address

# 6. No arguments -> interactive REPL
./ip-address-lookup-cli
ip-address-lookup> load 1 get_ip_address
ip-address-lookup> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-lookup-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-address-lookup-cli load 1 get_ip_address
   ```
...
```sh
./ip-address-lookup-cli load 1 get_ip_address          # scalar shorthand for {id:1}
./ip-address-lookup-cli load '{id:1}' get_ip_address     # explicit match map
```
...
./ip-address-lookup-cli load 1 get_ip_address
```
...
Run with no arguments to open a REPL (prompt `ip-address-lookup>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-address-lookup-cli
ip-address-lookup> load 1 get_ip_address
ip-address-lookup> /help
ip-address-lookup> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_ip_address`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-address-lookup-cli`. |
...
(`./ip-address-lookup-cli <expr>`) and interactively (the REPL), and expressions compose the
