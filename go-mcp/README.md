# ip-address-lookup-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpAddressLookup SDK as
two agent tools — `ip-address-lookup_list` and `ip-address-lookup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-address-lookup-mcp)
make build

# 2. Provide credentials via the environment
export IP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-address-lookup \
  -- /absolute/path/to/ip-address-lookup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-address-lookup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-address-lookup_list: first page of records
{ "entity": "get_ip_address" }
{ "entity": "get_ip_address", "query": { } }

// ip-address-lookup_load: one record by id
{ "entity": "get_ip_address", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-address-lookup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-address-lookup \
     -- "$PWD"/dist/*/ip-address-lookup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-address-lookup_list` and `ip-address-lookup_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_address using ip-address-lookup"*
   and it calls `ip-address-lookup_list` with `{"entity":"get_ip_address"}`.
...
```sh
export IP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx            # API key
export IP_ADDRESS_LOOKUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-address-lookup_list` tool
...
### Call the `ip-address-lookup_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-address-lookup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-address-lookup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_ADDRESS_LOOKUP_APIKEY` | API key sent with every request. |
| `IP_ADDRESS_LOOKUP_BASE` | Optional override of the API base URL. |
...
```sh
./ip-address-lookup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-address-lookup_load","arguments":{"entity":"get_ip_address","query":{"id":1}}}}'
```
