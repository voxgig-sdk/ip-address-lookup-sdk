...
// IpAddressLookup SDK GetIpAddress entity
...
class GetIpAddressEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetIpAddressEntity($this->_client, $opts);
    }

    /**
     * @param GetIpAddress|array $args GetIpAddress data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpAddressLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetIpAddress|array The current GetIpAddress data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetIpAddress fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpAddressLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetIpAddress fields).
     */
    ...
    /**
     * Load a single GetIpAddress.
     *
     * @param GetIpAddressLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetIpAddressLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetIpAddress|array The loaded GetIpAddress as an assoc-array at the
     *   SDK boundary; throws IpAddressLookupError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpAddressLookupHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
