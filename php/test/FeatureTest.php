...
// IpAddressLookup SDK feature test
//
...
class FtCtrl extends IpAddressLookupControl
{
...
class FtClient
{
    ...
    private IpAddressLookupUtility $utility;

    public function __construct(IpAddressLookupUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpAddressLookupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpAddressLookupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpAddressLookupContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new IpAddressLookupUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (IpAddressLookupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpAddressLookupConfig::make_feature($name);
    ...
    private static function buildUrl(IpAddressLookupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpAddressLookupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpAddressLookupResponse($fetched);
        }
    ...
    private function fail(IpAddressLookupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpAddressLookupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
