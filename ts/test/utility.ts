...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.IPADDRESSLOOKUP_TEST_LIVE ||
    'TRUE' === process.env.IPADDRESSLOOKUP_TEST_OVERRIDE
  ) {
  ...
  m.IPADDRESSLOOKUP_TEST_EXPLAIN = process.env.IPADDRESSLOOKUP_TEST_EXPLAIN || m.IPADDRESSLOOKUP_TEST_EXPLAIN
