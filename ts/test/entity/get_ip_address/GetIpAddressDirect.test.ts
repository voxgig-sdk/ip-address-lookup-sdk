...
describe('GetIpAddressDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new IpAddressLookupSDK({
  ...
  test('direct-load-get_ip_address', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_ip_address', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IP_ADDRESS_LOOKUP_TEST_GET_IP_ADDRESS_ENTID']
