...
//     (\`new IpAddressLookupSDK(...)\` / \`IpAddressLookupSDK.test(...)\` become
//     \`IpAddressLookupSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpAddressLookupSDK'
...
const TEST_SEED = {"entity":{"get_ip_address":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpAddressLookupSDK(...)\` and any
// \`IpAddressLookupSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpAddressLookupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpAddressLookupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
