...
module IpAddressLookupUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpAddressLookupResponse.new({})
    ...
    if fetch_err
      ...
      response = IpAddressLookupResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpAddressLookupResponse.new(fetched)
    else
