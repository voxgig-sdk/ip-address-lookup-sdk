...
// IpAddressLookup SDK retry feature
...
class IpAddressLookupRetryFeature extends IpAddressLookupBaseFeature
{
    ...
    public function init(IpAddressLookupContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpAddressLookupContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpAddressLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpAddressLookupContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
