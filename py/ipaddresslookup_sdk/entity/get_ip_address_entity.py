...
from ipaddresslookup_sdk.ipaddresslookup_types import (
    GetIpAddress,
...
class GetIpAddressEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetIpAddressEntity(self._client, opts)
    ...
    def data_get(self) -> GetIpAddress:
    ...
    def match_get(self) -> GetIpAddress:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetIpAddress:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetIpAddress().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
