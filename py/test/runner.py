# IpAddressLookup SDK test runner
...
class IpAddressLookupTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpAddressLookupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpAddressLookupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpAddressLookupTestRunner.getenv("IP_ADDRESS_LOOKUP_TEST_LIVE")
        override = IpAddressLookupTestRunner.getenv("IP_ADDRESS_LOOKUP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpAddressLookupTestRunner.getenv(key)
        ...
        explain = IpAddressLookupTestRunner.getenv("IP_ADDRESS_LOOKUP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_ADDRESS_LOOKUP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpAddressLookupTestRunner._test_control is not None:
            return IpAddressLookupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpAddressLookupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpAddressLookupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpAddressLookupTestRunner.load_test_control()
...
def load_env_local():
    IpAddressLookupTestRunner.load_env_local()


def env_override(m):
    return IpAddressLookupTestRunner.env_override(m)


def entity_data(v):
    return IpAddressLookupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpAddressLookupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpAddressLookupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpAddressLookupTestRunner.load_test_control()


def live_client_options():
    return IpAddressLookupTestRunner.live_client_options()


def live_delay_ms():
    return IpAddressLookupTestRunner.live_delay_ms()
